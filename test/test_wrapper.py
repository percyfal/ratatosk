...
class TestMiscWrappers(unittest.TestCase):
    ...
    def test_cutadapt(self):
        luigi.run(_luigi_args(['--target', os.path.basename(fastq1.replace(".fastq.gz", ".trimmed.fastq.gz")), '--config-file', localconf, '--parent-task', 'ratatosk.lib.files.fastq.FastqFileLink']), main_task_cls=CUTADAPT.CutadaptJobTask)
        luigi.run(_luigi_args(['--target', os.path.basename(fastq2.replace(".fastq.gz", ".trimmed.fastq.gz")), '--config-file', localconf, '--parent-task', 'ratatosk.lib.files.fastq.FastqFileLink']), main_task_cls=CUTADAPT.CutadaptJobTask)
        
    def test_fastqc(self):
        luigi.run(_luigi_args(['--target', os.path.basename(fastq1), '--config-file', localconf, '--parent-task', 'ratatosk.lib.files.fastq.FastqFileLink']), main_task_cls=FASTQC.FastQCJobTask)
...
class TestBwaWrappers(unittest.TestCase):
    ...
    def test_bwasampe(self):
        luigi.run(_luigi_args(['--target', sam, '--config-file', localconf, '--use-long-names']), main_task_cls=BWA.BwaSampe)
...
class TestPicardWrappers(unittest.TestCase):
    ...
    def test_picard_hsmetrics(self):
        luigi.run(_luigi_args(['--target', sortbam.replace(".bam", ".hs_metrics"), '--config-file', localconf, '--target-regions', targetregions, '--bait-regions', baitregions]), main_task_cls=PICARD.HsMetrics)
